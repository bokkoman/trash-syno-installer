...
gateway="$(ip route | grep "$(ip route get 1 | awk '{print $7}')" | awk 'FNR==2{print $1}')"
...
if wget -qO "$dockerdir/appdata/docker-compose.yml" https://raw.githubusercontent.com/bokkoman/trash-syno-installer/main/docker-compose.yml; then
...
sed -i "s/PUID=1035/PUID=$puid/g" "$dockerdir/appdata/.env"
printf '\n%s\n\n' "User ID set.."
