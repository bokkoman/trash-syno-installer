...
if ! synopkg is_onoff Docker > /dev/null 2>&1; then
    wget -qO ~/docker.spk "https://global.download.synology.com/download/Package/spk/Docker/20.10.3-1239/Docker-x64-20.10.3-1239.spk"
    synopkg install ~/docker.spk
    # test it
...
select opt in "${options[@]}"; do
    case $opt in
        ...
        "usenet")
            ...
            ;;
        "both")
            printf '\n%s\n' " You chose both torrents and usenet"
            printf '\n%s\n\n' " Creating data directories..."
            mkdir -p /volume1/data/{torrents/{tv,movies,music},media/{tv,movies,music}}
            mkdir -p /volume1/data/{usenet/{tv,movies,music},media/{tv,movies,music}}
